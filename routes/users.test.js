...
describe("POST /users", function () {
  test("ok for admins: create non-admin", async function () {
  ...
  test("ok for admins: create admin", async function () {
    const resp = await request(app)
      .post("/users")
      .send({
        username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("works for user with matching username", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
